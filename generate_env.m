function [task_list, auv_list, obstacle_list] = generate_env(num_tasks, num_obs, num_auvs, S, fail_num, entry_exit_split)
...
auv_list = struct([]);
fail_auv = randperm(S.fail, num_auvs, fail_num);  % 随机故障auv id
fail_time = 2000 * rand(S.fail,1,numel(fail_auv)) + 500; % 随机故障时间  500-2500s之间
for i = 1:num_auvs
    % AUV对每个任务的专长度
    expertises = rand(S.env, 1, num_tasks);      % 随机专长度0-1
    mileage = 15000;
    x = zeros(12,1);
...
for i = 1:num_tasks
    % 出入口点
    entry = [rand_range(S, xrange), rand_range(S, yrange), rand_range(S, zrange)];
    if entry_exit_split
        angle = rand()*2*pi;
        dist = rand_range(S,entry_exit_dist_range);
        offset = dist * [cos(angle), sin(angle), rand_range(S,[-0.5, 0.5])];
        exit = entry + offset;
    else
        exit = entry;
    end
    ...
    deadline = randi(S.env,[1000, 3000]);
    ...
    task_list(i).done = false;              % 是否已完成
    task_list(i).done_reward = 0;
...
count = 0;
while count < num_obs
    pos = [rand_range(S,xrange), rand_range(S,yrange), rand_range(S,zrange)];
    safe = true;
    for j = 1:num_tasks
...
%% 辅助函数
function val = rand_range(seed, range)
    val = range(1) + rand(seed.env) * (range(2) - range(1));
end
