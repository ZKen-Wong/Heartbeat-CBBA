function xdot = auv(x,ui)

%
% xdot = auv(x,ui);
%
% This function represents a state space model of an Autonomous
% Underwater Vehicle (AUV) which returns the time derivated of 
% the state vector :
%
% x   = [ u v w p q r xpos ypos zpos phi delta psi ]'  where
%
...
%
%
% Reference : Healey, A.J. and Lienard, D. (1993).
...
for xL = 0:dxL:L
Ucf = sqrt((v+xL*r)^2+(w-xL*q)^2);
Cyt = (0.5*0.6*(v+xL*r)^2+0.6*(w-xL*q)^2)*(v+xL*r)/Ucf;
Cy = Cy + dxL*Cyt;
end

for xL = 0:dxL:L
Ucf = sqrt((v+xL*r)^2+(w-xL*q)^2);
Czt = (0.5*0.6*(v+xL*r)^2+0.6*(w-xL*q)^2)*(w-xL*q)/Ucf;
Cz = Cz + dxL*Czt;
end

for xL = 0:dxL:L
Ucf = sqrt((v+xL*r)^2+(w-xL*q)^2);
Cmt = (0.5*0.6*(v+xL*r)^2+0.6*(w-xL*q)^2)*(w+xL*q)/Ucf*xL;
Cm = Cm + dxL*Cmt;
end

for xL = 0:dxL:L
Ucf = sqrt((v+xL*r)^2+(w-xL*q)^2);
Cnt = (0.5*0.6*(v+xL*r)^2+0.6*(w-xL*q)^2)*(v+xL*r)/Ucf*xL;
Cn = Cn + dxL*Cnt;
end
...
% Dimensional state derivatives ( xdot = Minv*f(x) )
